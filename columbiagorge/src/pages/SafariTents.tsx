...
const detail = accommodationTypeDetails['safari-tent'];

const SafariTents = () => {
  const [accommodations, setAccommodations] = useState<Accommodation[]>([]);
  const [lightboxOpen, setLightboxOpen] = useState(false);
  const [lightboxIndex, setLightboxIndex] = useState(0);
  const galleryImgs = detail.galleryImages.map((src, i) => ({ src, alt: `Safari tent ${i + 1}` }));

  useEffect(() => {
    api.get<{ accommodations: Accommodation[] }>('/accommodations')
      .then((d) => setAccommodations(d.accommodations.filter((a) => a.type === 'safari-tent')))
      .catch(console.error);
  }, []);

  return (
    <div>
      <PageHero image={detail.heroImage} subLabel="Our Sanctuaries" title="The Woodland Safari Tents" description={detail.tagline} />
      ...
      <section style={{ padding: 'clamp(4rem, 8vw, 7rem) 2rem', maxWidth: 900, margin: '0 auto' }}>
        {detail.longDescription.map((p, i) => (
          <motion.p key={i} variants={FADE_UP} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}
            style={{ fontFamily: 'var(--font-body)', fontSize: '1.05rem', lineHeight: 1.8, color: 'var(--color-gray)', marginBottom: '1.5rem' }}>{p}</motion.p>
        ))}
      </section>
      ...
      <section style={{ padding: '0 2rem 4rem', maxWidth: 1200, margin: '0 auto' }}>
        <motion.h2 variants={FADE_UP} initial="hidden" whileInView="visible" viewport={{ once: true }}
          style={{ fontFamily: 'var(--font-heading)', fontWeight: 300, fontSize: 'clamp(1.8rem, 3vw, 2.4rem)', textAlign: 'center', marginBottom: '2rem', color: 'var(--color-primary)' }}>Gallery</motion.h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1rem' }}>
          {detail.galleryImages.map((src, i) => (
            <motion.img key={i} src={src} alt={`Safari tent ${i + 1}`} variants={FADE_UP} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}
        ...
        <Lightbox images={galleryImgs} currentIndex={lightboxIndex} isOpen={lightboxOpen}
          onClose={() => setLightboxOpen(false)} onNext={() => setLightboxIndex((i) => (i + 1) % galleryImgs.length)} onPrev={() => setLightboxIndex((i) => (i - 1 + galleryImgs.length) % galleryImgs.length)} />
      </section>

      <section style={{ background: 'var(--color-slate)', padding: 'clamp(3rem, 6vw, 5rem) 2rem' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <motion.h2 variants={FADE_UP} initial="hidden" whileInView="visible" viewport={{ once: true }}
            style={{ fontFamily: 'var(--font-heading)', fontWeight: 300, fontSize: 'clamp(1.8rem, 3vw, 2.4rem)', textAlign: 'center', marginBottom: '2.5rem', color: 'var(--color-primary)' }}>What's Included</motion.h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1.5rem' }}>
            {detail.amenities.map((a, i) => (
              <motion.div key={i} variants={FADE_UP} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}
                ...
                <span style={{ fontSize: '1.5rem' }}>{a.icon}</span><span>{a.label}</span>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: 'clamp(4rem, 8vw, 6rem) 2rem', maxWidth: 1200, margin: '0 auto' }}>
        <motion.h2 variants={FADE_UP} initial="hidden" whileInView="visible" viewport={{ once: true }}
          style={{ fontFamily: 'var(--font-heading)', fontWeight: 300, fontSize: 'clamp(1.8rem, 3vw, 2.4rem)', textAlign: 'center', marginBottom: '3rem', color: 'var(--color-primary)' }}>Choose Your Safari Tent</motion.h2>
          ...
          {accommodations.map((acc, i) => (
            <motion.div key={acc.id} variants={FADE_UP} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}
              ...
              <div style={{ overflow: 'hidden', height: 220 }}>
                <img src={acc.images[0] ?? detail.heroImage} alt={acc.name} style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.4s' }}
      ...
      <SectionCTA heading="Where Canvas Meets Comfort" description="The thrill of the wild with the refinement of a boutique retreat."
        primaryLabel="Check Availability" primaryTo="/book" secondaryLabel="View All Stays" secondaryTo="/accommodations" />
    </div>
  );
};

export default SafariTents;

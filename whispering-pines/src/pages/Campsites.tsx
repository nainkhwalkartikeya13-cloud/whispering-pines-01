...
const detail = accommodationTypeDetails['campsite'];

const Campsites = () => {
  const [accommodations, setAccommodations] = useState<Accommodation[]>([]);
  const [lightboxOpen, setLightboxOpen] = useState(false);
  const [lightboxIndex, setLightboxIndex] = useState(0);
  const galleryImgs = detail.galleryImages.map((src, i) => ({ src, alt: `Campsite ${i + 1}` }));

  useEffect(() => {
    api.get<{ accommodations: Accommodation[] }>('/accommodations')
      .then((d) => setAccommodations(d.accommodations.filter((a) => a.type === 'campsite')))
      .catch(console.error);
  }, []);

  return (
    <div>
      <PageHero image={detail.heroImage} subLabel="Our Sanctuaries" title="The Wilderness Campsites" description={detail.tagline} />
      ...
      <section style={{ padding: '0 2rem 4rem', maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1rem' }}>
          {detail.galleryImages.map((src, i) => (
            <motion.img key={i} src={src} alt={`Campsite ${i + 1}`} variants={FADE_UP} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}
      ...
      <section style={{ padding: 'clamp(4rem, 8vw, 6rem) 2rem', maxWidth: 1200, margin: '0 auto' }}>
        <motion.h2 variants={FADE_UP} initial="hidden" whileInView="visible" viewport={{ once: true }}
          style={{ fontFamily: 'var(--font-heading)', fontWeight: 300, fontSize: 'clamp(1.8rem, 3vw, 2.4rem)', textAlign: 'center', marginBottom: '3rem', color: 'var(--color-primary)' }}>Choose Your Campsite</motion.h2>
      ...
      <SectionCTA heading="Live Beneath the Canopy" description="The most immersive way to experience the ancient forest of the Gorge."
